...
for dataset in [0, 1]:
    ...
    for n_components in cluster_counts:
        ...
        k_means = GaussianMixture(n_components=n_components, random_state=0)
...
for dataset in [0, 1]:
    ...
    k_means = GaussianMixture(n_components=k, random_state=0)
    ...
    ax[dataset, 0].legend(title='Component Number')
    ax[dataset, 0].set_title(dataset_name + ' T-SNE Embedding With Component', fontsize=14)
...
fig.suptitle('GMM T-SNE Embeddings and Labels', fontsize=26)
plt.savefig('GMM_t-sne.png')
